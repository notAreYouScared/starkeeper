...
>
    @for ($i = 1; $i <= 5; $i++)
        <button
            type="button"
            x-on:mousemove="onMouseMove($event, {{ $i }})"
            x-on:mouseleave="hovered = 0"
            x-on:click="setRating(hovered || {{ $i }})"
            style="background:none;border:none;padding:0;cursor:pointer;position:relative;width:1.15rem;height:1.15rem;"
            title="{{ $i }} star{{ $i > 1 ? 's' : '' }}"
        >
            ...
            <svg
                ...
                viewBox="0 0 20 20"
                fill="currentColor"
                style="position:absolute;top:0;left:0;width:1.15rem;height:1.15rem;display:block;color:#6b7280;"
                aria-hidden="true"
            >
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
            </svg>
            {{-- Yellow foreground star: full or half-clipped --}}
            <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
                fill="currentColor"
                style="position:absolute;top:0;left:0;width:1.15rem;height:1.15rem;display:block;color:#facc15;"
                x-bind:style="{
                    display: (hovered ? hovered : rating) >= {{ $i - 0.5 }} ? 'block' : 'none',
                    clipPath: (hovered ? hovered : rating) >= {{ $i }} ? '' : 'inset(0 50% 0 0)'
                }"
                aria-hidden="true"
            >
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
